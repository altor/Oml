(** Sérialisation des objets du langage **)
module OmlMarshalF : sig 

  val dump : ?flags:string -> string -> 'a -> unit
  val serialize : ?flags:string -> 'a -> string
  val unserialize : ?offset:int -> string -> 'a
  val load : string -> 'a
